...
- 2026-06-01: 누적 변경을 patch `0.3.2`로 묶었다 — 검증 거짓 양성 수정, 스마트 버튼·키 해제·상태 자동감지(Phase 1), Host·Username 기억+자동완성(Phase 2), 포트 입력 스피너 제거, dev 실행 시 타이틀바 아이콘이 안 뜨던 리소스 경로 버그 수정(`parents[2]`). (배포는 사용자 확인 후 진행)
- 2026-06-01: 이전 접속 주소 기억 + 자동완성을 추가했다(Phase 2). `history.py`로 host/port/user 프로파일을 앱 설정폴더 JSON에 저장(비밀번호 미저장)하고, Host 입력칸을 편집가능 콤보+QCompleter(부분일치)로 바꿔 선택 시 port/user를 자동 채운다. 성공한 접속(test/register/unregister) 시 이력에 기억한다.

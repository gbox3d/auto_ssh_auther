...
| 등록 상태 자동 감지 | 키 선택 시 서버 등록 상태를 자동 감지해 버튼이 `Register`/`Unregister`로 전환, 서버 무응답·주소 미입력 시 비활성화 |
| 키 해제 | 원격 `authorized_keys`에서 선택한 키 제거 |
| 접속 정보 기억 | 이전 host/port/user를 기억해 Host·Username 자동완성(선택+직접 입력), Host 선택 시 Port/User 자동 채움 (비밀번호는 저장 안 함) |
